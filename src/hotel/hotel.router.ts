...
class HotelRouter {
    ...
    private configure() {
        ...
        this._router.get('/:hotelName', async (req: Request, res: Response, next:NextFunction) => {
            try {
        ...
        this._router.delete('/:hotelName', async (req:Request, res:Response, next:NextFunction) => {
            try{
                const deletedHotel = await this._controller.deleteByName(req.params.hotelName);
